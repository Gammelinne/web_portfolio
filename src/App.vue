<template>
    <v-app id="App" :theme="is_light ? 'light' : 'dark'">
        <v-app-bar app>
            <h3 @click="goTo('#App')" class="Title mx-5">{{ $t('App.Title') }}</h3>
            <v-spacer></v-spacer>

            <v-btn icon @click="is_light = !is_light">
                <v-icon>{{ is_light ? 'mdi-weather-night' : 'mdi-weather-sunny' }}</v-icon>
            </v-btn>
            <v-btn icon class="mx-5" @click="changeLanguage">
                <v-icon>mdi-translate</v-icon> {{ language === 'en' ? 'fr' : 'en' }}
            </v-btn>
            <v-app-bar-nav-icon v-if="!is_mobile" title="Part" :icon="drawer ? 'mdi-close' : 'mdi-menu'"
                @click="drawer = !drawer">
            </v-app-bar-nav-icon>
        </v-app-bar>
        <v-navigation-drawer location="right" :permanent="!is_mobile">
            <v-list-item v-for="item in items" :key="item.title" link min-height="66" @click="goTo(item.route)"
        ...
        <v-footer padless>
            <span>&copy; {{ new Date().getFullYear() }} - {{ $t('App.Title') }} </span>
            <v-spacer></v-spacer>
            <v-btn class="mx-1" icon href="https://www.github.com/gammelinne" target="_blank">
                <v-icon>mdi-github</v-icon>
            </v-btn>
            <v-btn class="mx-1" icon href="https://www.linkedin.com/in/kylian-renault/" target="_blank">
